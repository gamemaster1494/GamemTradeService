...
{    
    ...
    public class Bot
    {
        public string sNumberTradeFile;
        ...
        // Handles Crafting
        public CraftingHandler craftHandler;

        // Handles counting of inventory and notifying if low/high
        public InformHandler informHandler;

        // Handles Advertisements in Group Chats
        ...
        public Dictionary<ulong, SteamID> dReserved;
        public Dictionary<ulong, SteamID> dDonated;
        public TF2Currency myCurrency = new TF2Currency();

        public TF2Currency userCurrency = new TF2Currency();

        public string sItemRaffleSaveFile;

        public ItemRaffleData itemRaffleData;

        // The Steam Web API key.
        public string apiKey;
        public SteamID TalkingWith;
        public FriendActivity FriendActivity = new FriendActivity();

        private List<SteamID> FriendThing = new List<SteamID>();

        private string FriendsFolder = AppDomain.CurrentDomain.BaseDirectory + @"friends\";
        ...
        public void CreateFriendFile(SteamID friend)
        {
            if (!FriendActivity.FriendActivityInfo.ContainsKey(friend.ConvertToUInt64().ToString()))
            {
                FriendActivity.FriendActivityInfo.Add(friend.ConvertToUInt64().ToString(), DateTime.Now);
            }
            FriendActivity.UpdateFriendActivity(FriendsFolder + logOnDetails.Username + ".json", this.FriendActivity);
        }

        public void RemoveFriendFile(SteamID friend)
        {
            if (FriendActivity.FriendActivityInfo.ContainsKey(friend.ConvertToUInt64().ToString()))
            {
                FriendActivity.FriendActivityInfo.Remove(friend.ConvertToUInt64().ToString());
            }
            FriendActivity.UpdateFriendActivity(FriendsFolder + logOnDetails.Username + ".json", this.FriendActivity);
        }

        public void UpdateFriendTime(SteamID friend)
        {
            if (FriendActivity.FriendActivityInfo.ContainsKey(friend.ConvertToUInt64().ToString()))
            {
                FriendActivity.FriendActivityInfo[friend.ConvertToUInt64().ToString()] = DateTime.Now;
            }
            FriendActivity.UpdateFriendActivity(FriendsFolder + logOnDetails.Username + ".json", this.FriendActivity);
        }
        ...
        public void CheckBackpack()
        {
            ...
            foreach (KeyValuePair<ulong, SteamID> pair in dReserved)
            {
                Inventory.Item item = MyInventory.GetItem(pair.Key);
                if (item == null)
                {
                    list.Add(pair.Key);
                    GetUserHandler(pair.Value).OnMessage("removed_item", EChatEntryType.ChatMsg);
                }
            }
            foreach (ulong uID in list)
            {
                dReserved.Remove(uID);
            }
        }
        public void UnReserveAllByUser(SteamID id)
        {
            Dictionary<ulong, SteamID> newlist = new Dictionary<ulong, SteamID>();
            foreach (KeyValuePair<ulong, SteamID> pair in dReserved)
            {
                if (pair.Value != id)
                {
        ...
        public void RemoveAllFriends()
        {
            ...
            {
                bfound = false;
                foreach (ulong admin in Admins)
                {
                    if (friend.ConvertToUInt64().ToString() == admin.ToString())
                    {
                        bfound = true;
                    }
                }
                if (clsFunctions.OneScrapBotID.ContainsKey(friend) || clsFunctions.ScrapbankingBotsID.ContainsKey(friend) || clsFunctions.HatbankingBotID.ContainsKey(friend))
                    bfound = true;
                if (!bfound)
                {
                    SteamFriends.SendChatMessage(friend, EChatEntryType.ChatMsg, "I am sorry. My friends list is pretty full, so I am removing you. Be sure you joined the group!");
                    SteamFriends.RemoveFriend(friend);
                }
            }
        }
        public void RemoveUnactiveFriends(int DaysInactive = 2)
        {
            Dictionary<string, DateTime> newFriendActivity = new Dictionary<string, DateTime>();

            foreach (KeyValuePair<string, DateTime> pair in FriendActivity.FriendActivityInfo)
            {
                TimeSpan datathing = DateTime.Now.Subtract(pair.Value);
                if (DateTime.Now.Subtract(pair.Value).TotalDays >= DaysInactive && !IsUserAdmin(pair.Key))
                {
                    newFriendActivity.Add(pair.Key, pair.Value);
                }
            }
            lock (this)
            {
                foreach (KeyValuePair<string, DateTime> pair in newFriendActivity)
                {
                    this.SteamFriends.SendChatMessage(new SteamID(pair.Key), EChatEntryType.ChatMsg, "Sorry. you have been inactive for " + DaysInactive.ToString() + " days, and my friends list is getting full... You can add me again though! Cya =)");
                    this.SteamFriends.RemoveFriend(FindFriend(pair.Key));
                    this.FriendActivity.FriendActivityInfo.Remove(pair.Key);
                }
            }

            FriendActivity.UpdateFriendActivity(FriendsFolder + logOnDetails.Username + ".json", this.FriendActivity);
        }
        private SteamID FindFriend(string userID)
        {
            foreach (SteamID friend in friends)
            {
                if (friend.ConvertToUInt64().ToString() == userID)
                {
                    return friend;
                }
            }
            return new SteamID(userID);
        }
        public bool IsUserAdmin(string userID)
        {
            bool bfound = false;
            foreach (KeyValuePair<SteamID, string> pair in clsFunctions.OneScrapBotID)
            {
                if (pair.Key.ConvertToUInt64().ToString() == userID)
                    bfound = true;
            }

            foreach (KeyValuePair<SteamID, string> pair in clsFunctions.ScrapbankingBotsID)
            {
                if (pair.Key.ConvertToUInt64().ToString() == userID)
                    bfound = true;
            }

            foreach (KeyValuePair<SteamID, string> pair in clsFunctions.HatbankingBotID)
            {
                if (pair.Key.ConvertToUInt64().ToString() == userID)
                    bfound = true;
            }

            if (clsFunctions.BotsOwnerID.ConvertToUInt64().ToString() == userID)
                bfound = true;
        ...
        public void CloseTrade() 
        {
            try
            {
                ...
                {
                    clsFunctions.InviteUserToClan(this, CurrentTrade.OtherSID);
